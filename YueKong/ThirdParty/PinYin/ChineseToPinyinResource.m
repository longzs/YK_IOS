...
-(void)cacheObjec:(id<NSCoding>)obj forKey:(NSString *)key
{
    ...
    __block NSString* cachePath = cachePathForKey(_directory, key);
...
/// add by zsl
-(NSString*)getPinYinFirstLetterFromString:(NSString*)str{

    if (0 == str.length) {
        return nil;
    }
    NSString* found = [self getHanyuPinyinRecordFromCharWithChar:[str characterAtIndex:0]];
    if (2 < found.length) {
        return [[found substringWithRange:NSMakeRange(1, 1)] uppercaseString];
    }
    return nil;
}
